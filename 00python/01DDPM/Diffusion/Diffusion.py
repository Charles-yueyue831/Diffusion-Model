...
class GaussianDiffusionTrainer(nn.Module):
    def __init__(self, model, beta_1, beta_T, T):
        ...
        beta：\begin{aligned}x_t=\sqrt{1-\beta_t}x_{t-1}+\sqrt{\beta_t}\epsilon\quad\ldots(3)\\\text{;where}\ \epsilon\sim\mathcal{N}(0,I)\end{aligned}
        """
    ...
    def forward(self, x_0):
        """
        :param x_0: x_0.shape = [8, 3, 32, 32]
        """
        t = torch.randint(self.T, size=(x_0.shape[0],), device=x_0.device)  # t.shape = [8]
...
class GaussianDiffusionSampler(nn.Module):
    ...
    def p_mean_variance(self, x_t, t):
        ...
        eps = self.model(x_t, t)  # eps.shape = [8, 3, 32, 32]
